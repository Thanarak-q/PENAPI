...
function parseCurl(input) {
  ...
  for (; i < tokens.length; i++) {
    ...
    if (t === '-X' || t === '--request') {
      ...
      const d = next();
      if (d != null) dataArgs.push(urlencodeData(d));
    } else if (t === '-G' || t === '--get') {
...
// Apply curl's --data-urlencode encoding: `name=value` keeps the name and
// encodes the value; `=value` and bare `value` encode the whole content. The
// `@file` / `name@file` file forms can't be read client-side, so pass through.
function urlencodeData(arg) {
  if (arg.includes('@')) return arg;
  const eq = arg.indexOf('=');
  if (eq === -1) return encodeURIComponent(arg);
  const name = arg.slice(0, eq);
  const value = encodeURIComponent(arg.slice(eq + 1));
