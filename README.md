...
| **Attack Surface / Recon** | Passive static analysis of loaded endpoints and saved human request logs. Flags auth gaps, optional auth, IDOR/BOLA hints, risky params, mass-assignment fields, credentials carried in the URL, auth over plaintext HTTP, API keys in the query string, discouraged OAuth2 grants, GraphQL / file-upload / management-debug surfaces, sensitive data seen in logs, spec-quality drift, and shadow endpoints — without sending any request. |
