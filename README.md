...
| **Fuzzer / Brute** | Mark injection points with `§§` (wrap a value: `§admin§`) or the keyword `FUZZ`. Built-in payload sets (SQLi, XSS, traversal, cmdi, SSRF, NoSQLi, LFI, SSTI, XXE, CRLF, LDAP, XPath, GraphQL, open-redirect, email-injection, format-string/edge-case, usernames, passwords, auth-bypass, host-header, Prototype Pollution, Mass Assignment Fields, CORS Test Origins, Unicode / Encoding Bypass), a numeric range for IDOR enumeration, and custom wordlists. Concurrency + delay, live streaming, sortable results, anomaly highlighting. Risky runs require confirmation and server-side caps limit accidental floods. |
...
| **Secret Scanner** | From **Attack → Secret Scanner…**, paste a response body, JS bundle, or config file to detect leaked credentials — AWS/Google/GitHub/Slack/Stripe/SendGrid keys, JWTs, private-key blocks, basic-auth in URLs, bearer tokens, and generic assigned secrets. Matches are previewed with the middle redacted. Fully client-side; nothing is sent. |
